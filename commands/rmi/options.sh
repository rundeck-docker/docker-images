...
# Created: Thu Jul 30 09:58:41 PDT 2015
#
#/ usage: docker-image:rmi  --docker-cert-path <${DOCKER_CERT_PATH:-}>  --docker-host <${DOCKER_HOST:-}>  --docker-tls-verify <${DOCKER_TLS_VERIFY:-}>  --image <> [ --force <false>] [ --no-prune <false>] 
...
rerun_options_parse() {
    ...
    while [ "$#" -gt 0 ]; do
        ...
        case "$OPT" in
            ...
            --no-prune) rerun_option_check $# $1; NO_PRUNE=$2 ; shift ;;
            # help option
            -|--*?)
                rerun_option_usage
                exit 2
    ...
    [ -z "$NO_PRUNE" ] && NO_PRUNE="$(rerun_property_get $RERUN_MODULE_DIR/options/no-prune DEFAULT)"
